...
|------|-----------|---------------|
| 온라인 설문 | `온라인설문` | - |
| 유선(매월) | `유선(매월)` | `신용기`, `이연숙`, `이정혜`, `권기현`, `박지영` |
| 방문 모니터링 | `방문모니터링` | - |
| 2차 방문 | `2차방문` | - |

> **유선(매월)** 데이터는 **동시에 2개 시트**에 저장됩니다: 메인 시트 + 담당자 이름 시트
...
2. 스프레드시트 이름: `노인돌봄 모니터링 데이터`
3. 헤더는 스크립트가 자동 생성합니다 (수동 입력 불필요)
...
> **현재 배포 URL**: `https://script.google.com/macros/s/AKfycbzB5oAdzFrUhW0BEnqZn7ecLwWBllc-zQdZKtrYOwNSjgl2XPbQ2Yp8Qd1PygbPaRxikw/exec`

---

## 4단계: 앱에서 URL 설정

1. 모니터링 앱 실행 (`npm run dev`)
2. 우측 상단 **⚙️ 설정** 버튼 클릭
3. 복사한 **웹 앱 URL 붙여넣기**
4. **저장** 클릭
...
이제 데이터 입력 후 **저장** 버튼을 누르면:
- 모드에 맞는 시트에 자동 저장
- 유선(매월)은 담당자 시트에도 동시 저장
- 한국어 헤더로 직관적인 데이터 구분
...
| 증상 | 해결 방법 |
|------|-----------|
| 저장 안됨 | Apps Script 배포 후 "모든 사용자" 접근 권한 확인 |
| 권한 오류 | Apps Script 재배포 후 새 URL 사용 |
| 시트 미생성 | 첫 데이터 저장 시 자동 생성됨 |
