...
import { 
  ...
  Save, Copy, Settings, CloudUpload
} from 'lucide-react';
...
${formData.interviewer_opinion || '(작성되지 않음)'}`;
  ...
  const handleSave = async () => {
    ...
    const result = await sendToGoogleSheet(scriptUrl, formData);
  ...
  return (
    <div className="min-h-screen pb-24 text-slate-800">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <nav className="flex bg-black/20 p-1 rounded-xl overflow-x-auto scrollbar-hide">
              {(['유선(매월)', '1차 대면', '온라인설문', '2차 대면'] as MonitoringMode[]).map((mode) => {
        ...
        <div className="transition-all duration-300">
          ...
          {formData.mon_method === '1차 대면' && (
            <VisitMode 
              ...
              hypotheses={hypotheses}
              setHypotheses={setHypotheses}
              riskTargets={riskTargets} // Passed for selection
              showToast={showToast}
            />
          )}
      ...
      {/* Floating Action Buttons - Hide for Online Mode */}
      {formData.mon_method !== '온라인설문' && (
